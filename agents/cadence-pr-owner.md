...
- Resolve merge conflicts when `mergeStateStatus` is BEHIND, BLOCKED, or DIRTY; preserve branch intent; abort and report if intents conflict.
...
1. `gh pr view N --json url,headRefName,baseRefName,mergeable,mergeStateStatus,statusCheckRollup`; create worktree at sibling `{repo}-pr{N}` if missing.
...
5. If `mergeStateStatus` is BEHIND, BLOCKED, or DIRTY: merge base into head — prefer local `git fetch origin && git merge origin/{baseRefName}`; or `gh pr update-branch` then `git fetch origin && git merge origin/{headRefName}` (or ff-only pull) so local branch matches remote before push. Resolve conflicts; re-run checks. Rebase only if the user explicitly approves force-with-lease — default merge-only.
...
7. Settle loop — repeat until stop:
   ...
   - Poll `gh pr checks N --required`; fix failures; push when needed.
