...
2. **Plan** — pick an approach with eyes open. Note data flow, edge cases, failure modes, and the tests that would prove it works. Surface hidden assumptions instead of guessing past them. See `scope-and-dispatch` for parent routing before subagent fan-out. Single-thread decomposition: `break-work-into-verifiable-steps` or `/plan`. Multi-workstream coordination (disjoint scopes, explicit dependencies): delegate to `cadence-planner`, then partition fan-out per `parallel-workstreams`.
