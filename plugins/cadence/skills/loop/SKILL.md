...
2. **Plan** — pick an approach with eyes open. Note data flow, edge cases, failure modes, and the tests that would prove it works. Surface hidden assumptions instead of guessing past them.
...
5. **Test** — verify against reality, not intent. See `verify-with-runtime`.
6. **Ship** — clean commits, accurate description, fresh evidence at the moment of push. See `clean-commits`.
