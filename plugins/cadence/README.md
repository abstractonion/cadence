...
34 durable rules, plus Codex command-mode skill shims, organized into five groups:
...
Once installed, Codex reads `.codex-plugin/plugin.json` and loads all `SKILL.md` files under `skills/`. The original slash-command primers are also exposed as Codex skill shims with `cadence-*` names, for example `cadence-plan`, `cadence-verify`, and `cadence-ship`.
...
│   └── ... (34 rules + command shims)
├── plugins/cadence/             # Self-contained Codex plugin package, generated by sync.sh
...
│   └── sync.sh                  # regenerates rules/*.mdc and plugins/cadence
├── LICENSE                      # MIT
├── README.md
└── NOTICE.md                    # upstream-source attribution
```

`skills/` is the single source of truth for rule bodies. `rules/` is a generated Cursor-only directory: each `.mdc` carries the same body wrapped with Cursor-specific frontmatter (`globs` for stack-scoped rules, `alwaysApply: true` for workflow rules). `plugins/cadence/` is a generated self-contained Codex package because Codex marketplace installs expect plugin contents to live under `plugins/<name>/`. The manifests act as filters into the same content tree — Cursor reads `rules/`, Claude Code reads the root plugin, Codex reads `plugins/cadence/`, and SKILL.md-aware runtimes can read `skills/` directly.

## Customize

### Editing a principle

Edit the canonical body in `skills/<name>/SKILL.md`. Then regenerate generated shims and packages:

```bash
./scripts/sync.sh
```

This rewrites every `rules/*.mdc` from the matching `skills/*/SKILL.md` plus the per-rule frontmatter declared in `scripts/cursor-rules.json`, then refreshes the self-contained Codex package in `plugins/cadence/`. Requires `jq` (`brew install jq`).
